...
moist_adiabats = np.concatenate(
    (np.arange(-15.0, 10.1, 5.0), np.arange(12.0, 45.1, 2.0))
)
...
def plot_wrf(filename, lat, lon, time, output):
    ...
    i, j = wrf.ll_to_ij(
        map_proj, truelat1, truelat2, stand_lon, dx, dy, ref_lat, ref_lon, lat, lon
    )
    ...
    i_u, j_u = wrf.ll_to_ij(
        map_proj, truelat1, truelat2, stand_lon, dx, dy, ref_lat_u, ref_lon_u, lat, lon
    )
    i_v, j_v = wrf.ll_to_ij(
        map_proj, truelat1, truelat2, stand_lon, dx, dy, ref_lat_v, ref_lon_v, lat, lon
    )
...
def plot_sounding_data(filename, output):
    ...
    for k in np.arange(1, nk):
        p[k] = p[k - 1] * np.exp(
            (metconst.g * (z[k - 1] - z[k]))
            / (metconst.Rd * met.T((th[k] + th[k - 1]) / 2.0, p[k - 1]))
        )
...
def plot_sounding_data_uwyo(filename, output, stationID=0, date=None):
    ...
    p, z, qv, u, v, th = uwyo.transform_and_check_data(
        p, z, qv, wind_dir, wind_speed, th
    )
...
def plot_sounding_data_csv(filename, output):
    ...
    for k in range(nk):
        if wdir[k] == -9999.0 and wspd[k] == -9999.0:
            u[k] = v[k] = -9999.0
        else:
    ...
    for k in range(nk):
        if wdir[k] == -9999.0 and wspd[k] == -9999.0:
            kb = ke = k
            while kb >= 0 and wdir[kb] == -9999.0 and wspd[kb] == -9999.0:
                kb -= 1
            while ke <= nk - 1 and wdir[ke] == -9999.0 and wspd[ke] == -9999.0:
                ke += 1
...
def plot_old(x, y, z, time, th, p, qv, u, v, title, output):
    """Plots Skew-T using legacy positional coordinate format.

    Wraps :py:func:`plot` converting x,y coordinates to a location string.

    :parameter x: X coordinate (km)
    :parameter y: Y coordinate (km)
    ...
    :parameter u: U wind component array (m/s)
    :parameter v: V wind component array (m/s)
    :parameter title: Plot title
    :parameter output: Output filename
    """
...
def plot(loc, z, th, p, qv, u, v, output, time=None, title=None):
    ...
    try:
        ...
        print(f"Error calculating sounding stats, datablock omitted: {e}")
    ...
    # Adjust plot margins.
    plt.subplots_adjust(
        left=0.03, bottom=0.03, right=0.97, top=0.97, wspace=0.12, hspace=0.12
    )
...
def plot_wind_axes(axes):
    ...
    Draws a vertical line for wind barb plotting and disables axis decorations.
...
def plot_wind(axes, z, p, u, v, x=0):
    """Plot wind barbs at a given x position.
    ...
    :parameter u: U wind component array (m/s)
    :parameter v: V wind component array (m/s)
    :parameter x: X position for barb placement (default 0)
    """
    for i in np.arange(0, len(z), 1):
        if p[i] > pt_plot:
            plt.barbs(x, p[i], u[i], v[i], length=5, linewidth=0.5)
...
def plot_sounding(axes, z, th, p, qv, u=None, v=None):
    ...
    axes.semilogy(
        Td + skew(p), p, base=math.e, color=linecolor_Td, linewidth=linewidth_Td
    )
    axes.semilogy(
        T_parcel + skew(pcl["pp"]),
        pcl["pp"],
        base=math.e,
        color=linecolor_Parcel_T,
        linewidth=linewidth_Parcel_T,
    ...
    axes.semilogy(
        Twb + skew(p), p, base=math.e, color=linecolor_Twb, linewidth=linewidth_Twb
    )
    ...
    if pcl["lfcprs"] > 0:
        label_m(Tmax - 0.5, pcl["lfcprs"], "--LFC", axes)
    if pcl["lclprs"] > 0:
        label_m(Tmax - 0.5, pcl["lclprs"], "--LCL", axes)
    if pcl["elprs"] > 0:
        label_m(Tmax - 0.5, pcl["elprs"], "--EL", axes)
    if pcl["ptops"] > 0:
        label_m(Tmax - 0.5, pcl["ptops"], "--TOPS", axes)

    # plot labels for std heights
    for plvl in plevs_std:
        zlvl = pymeteo.interp.interp_height(z, p, plvl)
        label_m(Tmin - 0.5, plvl, str(int(zlvl)), axes)
    ...
    if u is not None and v is not None:
        ...
        for i in np.arange(0, len(z), 2):
            if p[i] > pt_plot:
                plt.barbs(Tmin + 4, p[i], u[i], v[i], length=5, linewidth=0.5)


def plot_wind_barbs(axes, z, p, u, v):
    """Plot wind barbs along a vertical profile.
    ...
    for i in np.arange(0, len(z)):
        if p[i] > pt_plot:
            plt.barbs(0, p[i], u[i], v[i], length=5, linewidth=0.5)
...
def plot_hodograph(axes, z, u, v):
    ...
    for zlvl in np.arange(0, 7000, 1000):
        ...
        try:
            ...
            print(
                f"Error calculating sounding stats, storm motion marker not plotted: {e}"
            )


def calc_sounding_stats(_z, _th, _p, _qv):
    """Calculate sounding statistics for surface, most-unstable and mixed-layer parcels.

    :parameter _z: Height array (m)
    :parameter _th: Potential temperature array (K)
    :parameter _p: Pressure array (Pa)
    :parameter _qv: Water vapor mixing ratio array (kg/kg)
    :returns: Tuple of (surface_parcel, most_unstable_parcel, mixed_layer_parcel) dicts
    """
    T = met.T(_th, _p)  # T (K)
...
def calc_hodograph_stats(_z, _u, _v):
    """Calculate hodograph-derived statistics (shear, SRH, storm motion).

    :parameter _z: Height array (m)
    :parameter _u: U wind component array (m/s)
    :parameter _v: V wind component array (m/s)
    :returns: Dictionary with keys: bunkers, srh01, srh03, erh01, erh03,
        s01, s03, s06, s12, s23, s34, s45, s56
    """
    ...
    try:
        ...
        # SRH
        srh01 = dyn.srh(_u, _v, _z, 0.0, 1000.0, ucb[0], ucb[1])
        srh03 = dyn.srh(_u, _v, _z, 0.0, 3000.0, ucb[0], ucb[1])

    except (ValueError, ZeroDivisionError, IndexError) as e:
        print(f"Error calculating storm motion: {e}")
        ucb = [0.0, 0.0, 0.0, 0.0]
        srh01 = 0.0
        srh03 = 0.0

    try:
        erh01 = dyn.srh(_u, _v, _z, 0.0, 1000.0, 0.0, 0.0)
        erh03 = dyn.srh(_u, _v, _z, 0.0, 3000.0, 0.0, 0.0)

    except (ValueError, ZeroDivisionError, IndexError) as e:
        print(f"Error calculating erh: {e}")
        erh01 = 0.0
        erh03 = 0.0

    shear01 = dyn.uv_to_deg(*dyn.shear(_u, _v, _z, 0.0, 1000.0))
    shear03 = dyn.uv_to_deg(*dyn.shear(_u, _v, _z, 0.0, 3000.0))
    shear06 = dyn.uv_to_deg(*dyn.shear(_u, _v, _z, 0.0, 6000.0))
    shear12 = dyn.uv_to_deg(*dyn.shear(_u, _v, _z, 1000.0, 2000.0))
    shear23 = dyn.uv_to_deg(*dyn.shear(_u, _v, _z, 2000.0, 3000.0))
    shear34 = dyn.uv_to_deg(*dyn.shear(_u, _v, _z, 3000.0, 4000.0))
    shear45 = dyn.uv_to_deg(*dyn.shear(_u, _v, _z, 4000.0, 5000.0))
    shear56 = dyn.uv_to_deg(*dyn.shear(_u, _v, _z, 5000.0, 6000.0))
...
def plot_datablock(ax4, _x, _z, _t, _th, _p, _qv, _u, _v, _title):
    """Plot the sounding data information block with parcel stats and shear info.

    :parameter ax4: The axes to draw on
    :parameter _x: Location string
    :parameter _z: Height array (m)
    :parameter _t: Time value
    :parameter _th: Potential temperature array (K)
    :parameter _p: Pressure array (Pa)
    :parameter _qv: Water vapor mixing ratio array (kg/kg)
    :parameter _u: U wind component array (m/s)
    :parameter _v: V wind component array (m/s)
    :parameter _title: Plot title string
    """
    ...
    line = r"Estimated storm motion (supercell right mover) -> {0:3d}$^\circ$ {1:3.1f} m/s".format(
        int(cth), cr
    )
    plt.text(
        -1,
    ...
    print_parcel_info("Surface Parcel", pcl, -1.0, 0.65)
    print_parcel_info("Most Unstable Parcel", mupcl, -0.3, 0.65)
    print_parcel_info("500 m Mixed Layer Parcel", mlpcl, 0.4, 0.65)
    ...
    line = "Hodograph"
    plt.text(
        x, y, line, verticalalignment="center", horizontalalignment="left", fontsize=5
    )
    x += 0.02
    y -= 0.065
    line = r"0-1 km shear {0:3d}$^\circ$ {1:3.1f} m/s".format(
    ...
    y -= 0.05
    line = r"0-3 km shear {0:3d}$^\circ$ {1:3.1f} m/s".format(
    ...
    y -= 0.05
    line = r"0-6 km shear {0:3d}$^\circ$ {1:3.1f} m/s".format(
    ...
    y -= 0.05
    line = r"1-2 km shear {0:3d}$^\circ$ {1:3.1f} m/s".format(
    ...
    y -= 0.05
    line = r"2-3 km shear {0:3d}$^\circ$ {1:3.1f} m/s".format(
    ...
    y -= 0.05
    line = r"3-4 km shear {0:3d}$^\circ$ {1:3.1f} m/s".format(
    ...
    y -= 0.05
    line = r"4-5 km shear {0:3d}$^\circ$ {1:3.1f} m/s".format(
    ...
    y -= 0.05
    line = r"5-6 km shear {0:3d}$^\circ$ {1:3.1f} m/s".format(
...
def print_3col(name, value, unit, x, y):
    """Print a three-column text entry (name, value, unit) on the plot.

    :parameter name: Label text
    ...
    """
    plt.text(
        x, y, name, verticalalignment="center", horizontalalignment="left", fontsize=5
    )
    plt.text(
        x + 0.35,
        y,
...
def print_parcel_info(title, pcl, x, y):
    """Print parcel statistics (CAPE, CIN, TOPS, etc.) on the plot.

    :parameter title: Section title (e.g. 'Surface Parcel')
    :parameter pcl: Parcel statistics dictionary from :py:func:`pymeteo.thermo.CAPE`
    :parameter x: X position
    :parameter y: Y position
    """
    plt.text(
        x, y, title, verticalalignment="center", horizontalalignment="left", fontsize=5
    )
    y -= 0.065
    x += 0.02
    ...
    y -= 0.05
    print_3col("Parcel", "{0}".format(int(pcl["prs"] / 100.0)), "mb", x, y)


def remove_tick_labels(axes):
    """Remove tick labels from both axes.

    :parameter axes: The axes to modify
    """
...
def set_fontscalefactor(newfactor):
    """Set the global font scale factor for plot labels.

    :parameter newfactor: New font scale multiplier
    """
...
def label(x, y, s, c, r, axes):
    """Draw a bold label on the Skew-T at pressure level y (in hPa).

    :parameter x: X position (temperature-axis units)
    :parameter y: Y position (pressure in hPa; multiplied by 100 internally)
    :parameter s: Label string
    :parameter c: Font color
    :parameter r: Rotation angle (degrees)
    :parameter axes: The axes to draw on
    """
    axes.text(
        x,
...
def label_h(x, y, s, c, r, axes):
    """Draw a normal-weight label on the hodograph at position (x, y).

    :parameter x: X position (m/s)
    :parameter y: Y position (m/s)
    :parameter s: Label string
    :parameter c: Font color
    :parameter r: Rotation angle (degrees)
    :parameter axes: The axes to draw on
...
def label_h2(x, y, s, c, r, axes):
    """Draw a bold label on the hodograph at position (x, y).
...
def label_m(x, y, s, axes):
    """Draw a small right-aligned marker label on the Skew-T.

    Used for LCL/LFC/EL labels and height annotations.

    :parameter x: X position (temperature-axis units)
    :parameter y: Y position (pressure in Pa)
    :parameter s: Label string
    :parameter axes: The axes to draw on
    """
    axes.text(
        ...
        verticalalignment="center",
        horizontalalignment="right",
        fontsize=4 * fontscalefactor,
    )


# draw hodograph u-v space
def draw_hodograph(axes, bounds):
    """Draw hodograph background grid with speed rings and axis labels.

    :parameter axes: The axes to draw on
    :parameter bounds: List of [xmin, xmax, ymin, ymax] in m/s
    """
    xmin, xmax, ymin, ymax = bounds
    gmax = max(xmax, ymax)
    gmin = min(xmin, ymin)
    # draw u-v grid
    axes.plot([gmin, gmax], [0, 0], color="black", linewidth=0.5)
    axes.plot([0, 0], [gmin, gmax], color="black", linewidth=0.5)
    ...
    for u in np.arange(5, math.sqrt(xmax**2 + ymax**2) + 1, 5):
        axes.plot(*hodograph_circle(u), color="grey", linewidth=0.3)


# helper function for circle points
def hodograph_circle(r):
    """Compute x, y coordinates for a circle of radius *r*.

    Used to draw concentric wind-speed rings on hodograph plots.

    :parameter r: Radius of the circle (m/s).
    :returns: Tuple of (x, y) numpy arrays tracing the circle.
    """
    phi = np.arange(0.0, 2 * math.pi, 0.01)
    return r * np.cos(phi), r * np.sin(phi)


# draws a vertical line axis upon which to draw wind barbs
def draw_wind_line(axes):
    """Draw a vertical pressure axis for plotting wind barbs.

    Draws a thin black vertical line at x=0 on the given axes and marks
    standard pressure levels with small dots.

    :parameter axes: Matplotlib axes object for the wind-barb panel.
    """
    wind_line = []
    for p in plevs_plot:
        wind_line.append(0)
    axes.semilogy(wind_line, plevs_plot, color="black", linewidth=0.5)
...
def draw_isotherms(axes):
    ...
    for T in np.arange(-40, 40, 10):
        label(T + skew(87500), 875, str(T), "red", 90.0 - skew_angle, axes)
    for T in np.arange(-100, -20, 10):
        label(T + skew(17500), 175, str(T), "red", 90.0 - skew_angle, axes)
...
def draw_isobars(axes):
    ...
    for i in np.arange(1000, 100, -50):
        label(-10 - ((1000 - i) * 0.025), i, str(i), "black", 0, axes)
...
def draw_dry_adiabat(axes):
    ...
    for T in dry_adiabats:
        ...
        if T % 10 == 0:
            axes.semilogy(
                dry_adiabat, plevs_plot, base=math.e, color=lc_major, linewidth=lw_major
            )
        else:
            axes.semilogy(
                dry_adiabat, plevs_plot, base=math.e, color=lc_minor, linewidth=lw_minor
            )

    for T in np.arange(-20, 150, 10):
        p = (600.0 - 3.5 * T) * 100.0
        x = met.T(T + met.T00, p) - met.T00 + skew(p)
        x1 = met.T(T + met.T00, p + 0.5 * dp_plot) - met.T00 + skew(p + 0.5 * dp_plot)
        x2 = met.T(T + met.T00, p - 0.5 * dp_plot) - met.T00 + skew(p - 0.5 * dp_plot)
        dx = x2 - x1
...
def draw_water_mix_ratio(axes):
    ...
    for W in mixing_ratios:
        ...
        axes.semilogy(
            water_mix, ps, base=math.e, color="grey", linestyle="--", linewidth=0.5
        )

        # Label the isoline
        T = TMR(W, 1075.0)
        label(T + skew(107500.0), 1075, str(W), "black", -15, axes)
...
def TMR(W, p):
    """Compute the temperature on a given mixing-ratio line at pressure *p*.

    Uses an empirical polynomial approximation to find the temperature (in
    degrees Celsius) at which the saturation mixing ratio equals *W* at
    pressure *p*.  Used for plotting mixing-ratio lines on the Skew-T.

    :parameter W: Mixing ratio in g/kg dry air.
    :parameter p: Pressure in millibars (hPa).
    :returns: Temperature in degrees Celsius.
    """
    # TODO: change this to something else?
    x = np.log10(W * p / (622 + W))
    TMR = (
        10 ** (0.0498646455 * x + 2.4082965)
        - 280.23475
        + 38.9114 * ((10 ** (0.0915 * x) - 1.2035) ** 2)
    )
    return TMR
